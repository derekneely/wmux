 ...
 * behind `wmux current-workspace` (issue #5).
 *
 ...
 * (the regression `list-workspaces`' `isActive` would cause), and an
 * unresolvable surface must be an explicit miss rather than the focused
 * workspace — an agent has to be able to tell "I don't know" from "it's this
 * one".
 */
...
describe('workspace.current (issue #5)', () => {
