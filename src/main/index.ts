...
function handleHookEvent(params: any): void {
  ...
  // The event is resolved, not read: a PostToolUse payload names its tool and
  // no event at all, so gating on `params.event` dropped every one of them.
  // See hookEventName for what that cost.
  const hookEvent = hookEventName(params);
  if (params?.surfaceId && hookEvent) {
    applyHookToAgentState(params.surfaceId as SurfaceId, hookEvent, params.message ?? null);
  }
