...
const SPECS: Record<string, BridgeSpec> = {
  ...
  // The caller's own workspace (issue #5). `callerScoped` supplies the id from
  // the caller's surface; when that resolution misses (no WMUX_SURFACE_ID, or a
  ...
  // explicit miss, never the focused workspace — guessing here is the same
  // silent-wrong-answer trap as `workspace.list`'s `isActive`.
  'workspace.current': {
