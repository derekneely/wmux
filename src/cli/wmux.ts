...
/**
 * The workspace this shell lives in (issue #5). An agent knows its surface
 * (`WMUX_SURFACE_ID`) but had no way to name the workspace holding it, and
 * `list-workspaces`' `isActive` answers about the FOCUSED workspace — the wrong
 * one for every background pane, silently. `--surface <id>` asks on another
 * pane's behalf. A surface that resolves to nothing is an error, not a guess.
 */
